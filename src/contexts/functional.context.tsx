...
const FunctionalContext = React.createContext<FunctionalStateModel>(initialState);
...
const FunctionalContextProvider = ({ children }: Props) => {
  ...
  const changeColor = (value: string, index: number) => {
    return dispatch({ type: ActionModel.CHANGE_COLOR, payload: { value, index } });
  };
  ...
  const handlePercentValues = (index: number, position: string, value: string) => {
    return dispatch({ type: ActionModel.LENGTH, linear_length: { index, position, value } });
  };

  const changePosition = (p_x: string, p_y: string, l_x: string, l_y: string) => {
    return dispatch({ type: ActionModel.POSITION, position: { x: p_x, y: p_y }, radial_length: { x: l_x, y: l_y } });
  };
    ...
    switch (type) {
      ...
      case NavDataIdType.ADD:
        if (state.colors.length < 8 && state.colors.length % 2 !== 0) {
          return dispatch({ type: ActionModel.ADD_COLOR, payload: setColor.roseColor });
        } else if (state.colors.length < 8 && state.colors.length % 2 === 0) {
          return dispatch({ type: ActionModel.ADD_COLOR, payload: setColor.orchidColor });
        } else {
  ...
  useEffect(() => {
    if (state.prefix.includes('linear')) {
      ...
      setGradient(`${state.prefix}(${state.radial_length.x}% ${state.radial_length.y}% at ${state.position.x}% ${state.position.y}%, ${code.toUpperCase()});`);
    }
  }, [state.prefix, state.deg, state.position, state.radial_length.x, state.radial_length.y, code]);
